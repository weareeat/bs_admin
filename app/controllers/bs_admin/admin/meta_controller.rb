class BsAdmin::Admin::MetaController < AdminControllerBase
  ...
  def new
    ...
    @url = bs_admin.admin_bsadmin_create_url(@meta.base_path)
  end

  def edit
    @model = @meta.class.find(params[:id])
    @url = bs_admin.admin_bsadmin_update_url(@meta.base_path, params[:id])
  end
  ...
  def create
    if @meta.can :create
      ...
      if @model.save
        redirect_to bs_admin.admin_bsadmin_index_url(@meta.base_path), notice: "#{@meta.humanized_name} was successfully created."
      else
        render action: "new"
      end
    end
  ...
  def update
    if @meta.can :edit
      ...
      if @model.update_attributes(params[@meta.symbol])
        redirect_to bs_admin.admin_bsadmin_index_url(@meta.base_path), notice: "#{@meta.humanized_name} was successfully updated."
      else
        render action: "edit"
      end
    end
  ...
  def destroy
    if @meta.can :delete
      ...
      redirect_to bs_admin.admin_bsadmin_index_url(@meta.base_path)
    end
  end
  ...
  def bsadmin_initialize
    @meta = BsAdmin::Helpers.find params[:base_path]
  end
end
